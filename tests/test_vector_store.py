...
def test_chromadb_backend_fails_clearly_when_unavailable(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="ChromaDB was requested"):
        query_knowledge_base(conn, "local context", limit=5)
